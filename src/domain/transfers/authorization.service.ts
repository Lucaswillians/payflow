...
export class AuthorizationService {
  ...
  async authorize(payerId: number, payeeId: number, amount: number): Promise<boolean> {
    const url = 'https://util.devi.tools/api/v2/authorize';

    try {
      const response: AxiosResponse<AuthorizationResponse> = await axios.get(url, {
        params: { payerId, payeeId, amount },
      });
